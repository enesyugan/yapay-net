...
def beam_search(model, src_seq, src_mask, device, max_node=10, max_len=200, 
    ...
    encoder_outputs = model.encode(
				input_features=src_seq,
				attention_mask=src_mask,
				)

    encoder_outputs = dict(encoder_outputs)
    ...
    for step in range(max_len):
        ...
        for k in range(l):
            ...
            encoder_outputs["decoder_input_ids"] = seq
            decoder_probs = model.decode(**encoder_outputs)


            dec_out = decoder_probs
